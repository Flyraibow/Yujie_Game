...
void CPPClass::addVariable(const string &varName, const string &varType, bool isPrivate)
{
  ...
  this->addVariable(var, isPrivate);
}

void CPPClass::addVariable(CPPVariable *variable, bool isPrivate)
{
  if (isPrivate) {
    p_privateVariables.push_back(variable);
  } else {
    p_publicVariables.push_back(variable);
  }
}
...
string CPPClass::getClassContentString() const
{
  ...
  for (auto var : p_privateVariables) {
    if (var->isStatic()) {
      statement += var->getTypeString() + " " + this->getClassName() + "::" + var->getVarString() + " = " + var->getInitialValue() + ";\n";
    }
  }
  for (auto var : p_publicVariables) {
    if (var->isStatic()) {
